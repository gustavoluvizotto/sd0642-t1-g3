...
int main(int argc, char **argv) {
	...
	int server_id;
	int aux;
	struct timeval total_start; /* para medição do tempo total de execução da solução */
	struct timeval total_finish; /* para medição do tempo total de execução da solução */

	gettimeofday(&total_start, NULL); /* início da execução do programa */
	...
	gettimeofday(&total_finish, NULL); /* término da execução do programa */

	/* mostra o tempo total de execução deste código, da primeira linha de código até a última */
	printf("Tempo total Client%d: %s\n", host_id, elapsedtime(total_finish, total_start));
