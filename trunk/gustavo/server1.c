...
int main(void) {
	...
	char tsend[10];							/* tempo de envio */
        ...
        port_number_receive = 57531;                   	/* porta de comunicacao para recebimento do vetor */
...
/*------------Comunicando com o primeiro cliente--------------------------------------*/
	...
	for (i=0; i<TAM; i++) {			/* recebe o vetor */
...
/*----------------------enviando o vetor ja ordenado------------------------------------*/
	...
	if (connect(sockfd,(struct sockaddr *) &serv_addr,sizeof(serv_addr)) < 0)
 	...
 	n = write(sockfd,tsend,10);		/* envia para o servidor esse tempo */
        if (n < 0)    
		error("Error writing to socket");
        for (i=0; i<2*TAM; i++) {   		/* envia o vetor */
