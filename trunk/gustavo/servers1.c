...
int main(void) {
	...
	int v1[2*TAM], v2[2*TAM];					/* vetores recebidos dos clientes */
	int v3[4*TAM];							/* vetor ordenado */ 
	char *aux1[2*TAM], *aux2[2*TAM];				/* vetores aux. que recebem dos clientes */         
	...
	char *aux3[4*TAM];						/* vetor auxiliar para envio */
	...
	char tsend[10];							/* tempo de envio */
        ...
        port_number_receive = 57535;			/* porta de comunicacao para recebimento do vetor */
...
/*------------Comunicando com o primeiro cliente--------------------------------------*/
	...
	for (i=0; i<2*TAM; i++) {		/* recebe o vetor */
...
/*----------------------enviando o vetor ja ordenado------------------------------------*/
	...
	if (connect(sockfd,(struct sockaddr *) &serv_addr,sizeof(serv_addr)) < 0)      
        ...
        tsend_i = tempo();			/* captura o tempo antes de começar a enviar o vetor */
 	sprintf(tsend,"%f",tsend_i);		/* converte para string */
 	n = write(sockfd,tsend,10);		/* envia para o servidor principal esse tempo */
        if (n < 0)    
		error("Error writing to socket");
        for (i=0; i<4*TAM; i++) {   		/* envia o vetor */
