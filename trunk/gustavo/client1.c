...
int main(void){
	...
	char tcom[10];							/* tempo de comunicacao */
	
        port_number_send = 57531;	/* porta de comunicacao para envio do vetor */
        host = "venon";         	/* host que o client 1 e 2 irao se comunicar */
	...
	srand48(time(NULL));
        ...
        tord_i = tempo();	/* captura o tempo de ordenação */
        bubble(aux2,TAM);
        tord_f = tempo();
        
        tcom_i = tempo();		/* captura o tempo antes de começar a enviar o vetor */
	sprintf(tcom,"%f",tcom_i);	/* converte para string */
 	n = write(sockfd,tcom,10);	/* envia para o servidor esse tempo */
        if (n < 0)    
...
/*-----------------recebendo o vetor do servidor principal----------------------------------------------*/
        port_number_receive = 57538;                   	/* numero da porta que recebe do servidor principal*/
	...
	n = read(newsockfd,tcom,10);		/* le o tempo antes do servidor principal começar a enviar o vetor */
     		if (n < 0) 
	...
	tcom_f = tempo();		/* captura o tempo apos receber o vetor todo do servidor principal */
	tcom_i = strtod(tcom,NULL);	/* converte o tempo recebido do servidos principal para double */
	close(sockfd);
        close(newsockfd);
	for (i=0; i<8*TAM; i++) {
		v2[i] = atoi(aux[i]);
